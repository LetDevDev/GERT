...
icmp.find_routes = function(interface)
  interface:send(gert.utils.getBroadcastAddr(interface),1,"03")
...
local function onPacket(name,source,dest,proto,data)
  ...
  if data:sub(1,2) == "01" then
    gert.send(dest,1,"02"..data:sub(3))
  end
end
